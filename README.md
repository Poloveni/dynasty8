...
- **Un site public** (`public/*.html`) : Accueil, Intérieurs, Garages, Cohérences, Produits exclusifs, Services, Équipe, FAQ, et une fiche détaillée par annonce.
...
| Les descriptions des zones (Habitation, Garage, Cayo, Roxwood) | `public/index.html` et `public/coherences.html` | le tableau `ZONES` / les cartes `carte-zone` |
| Les services proposés | `public/services.html` | les blocs `carte-service` |

Pour ajouter des **photos** à une annonce depuis l'espace agents : mets ta capture d'écran dans un salon Discord privé (ou tout autre hébergeur d'images), fais un clic-droit dessus → « Copier le lien de l'image », puis colle ce lien dans le champ « Photos » du formulaire d'annonce (une adresse par ligne).
